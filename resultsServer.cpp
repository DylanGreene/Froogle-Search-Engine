...
int main(void)
{
	...
	//map<string, vector< pair<string, unsigned int> > > index = ind.getIndexer();
	...
	{
		auto urlFreqVector = it->second;
		for(int i = 0; i < urlFreqVector.size(); i++)
		{
			int weight = calcSearchTermWeight(urlFreqVector[i].second);
			urlRanking[urlFreqVector[i].first] += weight;
		}
	}
